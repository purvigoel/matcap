...
def generate(model, args):
    ...
    condition_patch = img
    mean_c1 = condition_patch[:, :, 0].mean()
    mean_c2 =  condition_patch[:, :, 1].mean()
    mean_c3 =  condition_patch[:, :, 2].mean()
    output_image = model(noise)[0].permute(1,2,0)
    output_image = output_image.clamp(min=-1, max=1)  
    output_image = (output_image - output_image.min())/(output_image.max() - output_image.min())
    output_image = output_image * torch.tensor([mean_c1, mean_c2, mean_c3])
    # output_image = ((output_image + 1.0) * 255 / 2.0)
    
    output_image = output_image * 256.0 
